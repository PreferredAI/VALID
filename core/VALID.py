...
class VALID(GeneralRecommender):
    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(VALID, self).__init__(config, dataset)
    ...
    def forward(self, rating_matrix):
        items = F.normalize(self.item_embedding.weight, dim=1)  # [n_items, d]
        cores = F.normalize(self.k_embedding.weight, dim=1).unsqueeze(dim=0)  # [K, d]
        x_latent = cores
        ...
        for n_iter in range(self.num_iters):
            ...
            # cross-attention
            logit = x_latent.matmul(items.transpose(0, 1)) / self.tau  # [bsz/1, K, n_items]
            logit = logit.transpose(1, 2)

            softmax_dim = -1
            if self.nogb:
                attn_score = torch.softmax(logit, dim=softmax_dim)  # softmax over K factors
            else:
                attn_score_sample = F.gumbel_softmax(logit, tau=1., hard=False, dim=softmax_dim)
                attn_score_mode = torch.softmax(logit, dim=softmax_dim)
                attn_score = self.training * attn_score_sample + (1 - self.training) * attn_score_mode
            # attn_score: [B, K, n_items]

            x_list = []
            for k in range(self.kfac):
                attn_score_k = attn_score[:, :, k]  # [bsz, n_items]
                x_k = rating_matrix * attn_score_k  # [bsz, n_items]
                if self.normalize_score:
                    x_k = F.normalize(x_k, dim=1, p=2)
                x_k = F.dropout(x_k, self.drop_out, training=self.training)
                if n_iter < self.num_iters - 1:
                    x_latent_k = F.normalize(x_k.matmul(items), dim=1)
                    x_list.append(x_latent_k.unsqueeze(dim=1))
                else:
                    # encoder
                    h = self.encoder(x_k)  # [bsz, 2 * emb_size]
                    mu = h[:, :self.embedding_size]
                    mu = F.normalize(mu, dim=1)
                    logvar = -h[:, self.embedding_size:]

                    mu_list.append(mu)
                    log_var_list.append(logvar)

                    z = self.reparameterize(mu, logvar)

                    # decoder
                    z_k = F.normalize(z, dim=1)
                    logits_k = torch.matmul(z_k, items.transpose(0, 1)) / self.tau_dec
                    probs_k = torch.exp(logits_k)
                    probs_k = probs_k * attn_score_k
        ...
        return logits, mu_list, log_var_list
